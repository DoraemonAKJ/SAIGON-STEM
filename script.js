...
const lessonsData = [
  ...
  },
  {
    ...
    grade: 5,
    lesson: 3,
    title: "Xe dò line tự động Yolo Uno",
    illu: "🏎️",
    downloaded: false,
  },
  ...
  },
  {
    ...
    grade: 3,
    lesson: 1,
    title: "Tìm kiếm thông tin hiệu quả",
    illu: "🔍",
    downloaded: false,
  },
  ...
  {
    id: 13,
    subject: "Chương trình khu vực 2",
    grade: 3,
    lesson: 4,
    title: "Dự án khám phá địa phương",
    illu: "⭐",
    downloaded: false,
  },
  {
    ...
    grade: 5,
    lesson: 2,
    title: "Giao lưu văn hóa vùng miền",
    illu: "⭐",
    downloaded: false,
  },
];
...
function highlightActiveSidebar() {
  ...
  sidebarNav.querySelectorAll(".nav-item").forEach((el) => {
    ...
    if (
      ...
      currentPage === "Chương trình khu vực 2" &&
      href.includes("khuvuc2")
    ) {
      el.classList.add("active");
    }
  });
    ...
    themeToggleBtn.addEventListener("click", () => {
      ...
      if (isDark) {
        ...
        themeToggleBtn.innerHTML =
          '<i class="fa-solid fa-moon"></i><span class="nav-label">Giao diện tối</span>';
      } else {
        document.body.setAttribute("data-theme", "dark");
        themeToggleBtn.innerHTML =
          '<i class="fa-solid fa-sun"></i><span class="nav-label">Giao diện sáng</span>';
      }
    });
  }
...
function renderLessons() {
  ...
  const filteredLessons = lessonsData.filter((item) => {
    const matchCategory =
      pageCategory === "all" || item.subject === pageCategory;
    const matchGrade =
      currentGrade === "all" || item.grade.toString() === currentGrade;
    const matchSearch =
      item.title.toLowerCase().includes(currentSearchText) ||
  ...
  grid.innerHTML = filteredLessons
    .map(
      (item) => `
